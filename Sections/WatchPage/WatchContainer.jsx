...
const WatchContainer = ({ episodes, animeId, animeInfoData, seasons }) => {
  ...
  useEffect(() => {
    ...
    const fetchServers = async episodeId => {
      try {
        ...
        setServers({ sub: data.sub, dub: data.dub });
  ...
  useEffect(() => {
    if (!servers || (!servers.sub.length && !servers.dub.length)) return;
  ...
  return (
    <>
      <div
        className="flex justify-center md:flex-row flex-col gap-4 md:px-6
        bg-backgroundtwo"
      >
...
-630 -500 l-574 -474 -5 432 c-6 480 -5 476 -77 535 -67 56 -151 65 -230 26z"
    ...
    md:flex"
                  ...
                  rounded-lg"
                    >
                      <span className="hidden md:flex text-[11px] pl-2">
                        Next
                      </span>
