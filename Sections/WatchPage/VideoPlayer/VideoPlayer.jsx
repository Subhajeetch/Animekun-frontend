...
const VideoPlayer = ({
  ...
  const [isShowNextEpisodeButton, setIsShowNextEpisodeButton] = useState(false);
  ...
  const initialSubtitleStyles = () => {
    ...
    if (settings) {
      ...
      const textOpacity = parseFloat(parsedSettings.textOpacity || "100") / 100;
      const bgOpacity = parseFloat(parsedSettings.bgOpacity || "50") / 100;

      const rgbaTextColor = `rgba(${
        colorMap[parsedSettings.textColor?.toLowerCase()] || "255, 255, 255"
      }, ${textOpacity})`;
  ...
  useEffect(() => {
    ...
    if (Hls.isSupported()) {
      ...
      const tracks = data.tracks.filter(track => track.kind === "captions");
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      if (
        ...
        (!exceptionRef.current || !exceptionRef.current.contains(event.target)) // Exception div
      ) {
        setIsDropdownOpen(false);
        setManualShow(false);
      }
  ...
  const handleSubtitleSettingsChange = newSettings => {
    ...
    const textOpacity =
      parseFloat((newSettings.textOpacity || "100%").replace("%", "")) / 100;
  ...
  const calculateNewTime = (() => {
    ...
    return clientX => {
      ...
      const percentage = Math.max(0, Math.min(offsetX / progressBarWidth, 1));
          ...
          {isDropdownOpen && (
            ...
            >
              <TabsList className="bg-background h-[37px] w-full">
                ...
                >
                  <Captions size={17} className="text-white" />
                </TabsTrigger>
                <TabsTrigger
                  value="videoSpeed"
                  className="bg-transparent
                ...
                >
                  <CircleGauge size={17} className="text-white" />
                </TabsTrigger>
                <TabsTrigger
                  value="subSetting"
                  className="bg-transparent
                ...
                >
                  <FolderCog size={17} className="text-white" />
                </TabsTrigger>
              </TabsList>
              <TabsContent value="quality" className="bg-backgroundtwo">
                {/* quality in tabssss */}
                ...
                <div className=" w-full max-h-[116px] md:max-h-[160px] overflow-y-auto">
                  <div className="flex flex-col mx-2">
                    ...
                    >
                      ...
                      {currentQuality === "auto" && (
                        <Check size={12} className="text-white ml-2" />
                      )}
                    </div>
              ...
              <TabsContent value="subtitle">
                ...
                <div className="mb-2 w-full max-h-[116px] md:max-h-[156px] overflow-y-auto">
                  ...
                  >
                    <span className="flex-1 text-[10px]">Off</span>
                    {currentSubtitleTrack === -1 && (
                      <Check size={12} className="text-white ml-2" />
                    )}
                  </div>
                  ...
                  {subtitleTracks.map((track, index) => (
                    ...
                    >
                      <span className="flex-1 text-[10px]">{track.label}</span>
                      {currentSubtitleTrack === index && (
                        <Check size={12} className="text-white ml-2" />
                      )}
                    </div>
              ...
              <TabsContent value="subSetting">
                ...
                <SubtitleSettings
                  className="w-max"
                  handleSubtitleSettingsChange={handleSubtitleSettingsChange}
          ...
          <div className="flex items-center w-full px-2 mb-1">
            <div className="flex gap-[12px] items-center flex-none">
              ...
              <div
                ...
                ml-2 "
              >
                {isPlaying ? <Pause size={22} /> : <Play size={18} />}
              </div>
              ...
              >
                {isMuted ? <Muted size={18} /> : <Unmuted size={21} />}
              </div>
            </div>
            ...
            <div className="flex gap-[24px] items-center flex-none">
              ...
              <div className="flex items-center">
                ...
                cursor-pointer"
                  >
                    <SlidersVertical size={17} className="text-white" />
                  </div>
                </div>
